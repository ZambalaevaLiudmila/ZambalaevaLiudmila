--create database filmsytezla;
--set datestyle to iso,dmy;
--create table country (id serial not null primary key, place_name varchar); 
--insert into country (place_name) values 
--('Россия'), ('США'), ('Китай'), ('Корея'), ('Индия'), ('Япония'), ('Франция') ;
--create table directors (id serial primary key, full_name varchar, birthday date, reward text) ; 
...
--create table grade (id int not  null primary key, title varchar) ;
--insert into grade (id, title) 
--values (-3,  'horrible'), (-2, 'bad'), (-1, 'not good'), (0, 'boring'), (1, 'not bad'), (2, 'good'), 
--(3, 'excellent') ;
--create table genre (id serial primary key, title_of_genre varchar, description_of_genre text) ; 
--insert into genre (title_of_genre, description_of_genre) values ('приключения', 'Приключенческие фильмы – жанр 
...
--create table film_list (id serial not null primary key, title varchar, description text, 
--country int references country (id) , director int references directors(id) , actors text,
--year int, film_genre int references genre(id) , film_grade int references grade(id));
--insert into film_list (title, description, country, director, actors, year, film_genre, film_grade) 
--values ('Бивень (2014)','Журналист Уоллес Брайтон уехал брать интервью и исчез навсегда.
...
--create table subscription (id serial not null primary key, title varchar, description text,
--price numeric (10, 2));
--insert into subscription (title, description, price) values ('one month', 'полный доступ ко всем
-- фильмам на один месяц', 12), ('three months','полный доступ ко всем фильмам на 3 месяца', 35), 
--('half year', 'полный доступ ко всем фильмам на полгода', 67), ('nine months', 'полный доступ ко 
--всем фильмам на 9 месяцев', 95), ('full year', 'полный доступ ко всем фильмам на целый год', 120); 
--create table news (id serial primary key, title varchar, news text, date_of_news date, 
--film_tag int references film_list(id)); 
--insert into news (title, news, date_of_news, film_tag) values 
...
--create table private_office (id uuid DEFAULT gen_random_uuid() primary key , 
--name varchar, surname varchar, birth date, email varchar, user_subscription int references subscription(id) , 
--payment boolean, favorite_films int references film_list(id) ); 
--insert into private_office (name , surname , birth , email, user_subscription , payment , favorite_films)
--values ('Aleksandr', 'Petrov', '01/02/1985', 'petrov85al@mail.ru', 1, 'yes', 5),
...
--create table comments (id serial not null primary key, user_name uuid DEFAULT gen_random_uuid() references private_office(id) ,
--film int references film_list(id) , grade int references grade(id), comment text) ;
--insert into comments (user_name, film, grade, comment) values 
--((select id from private_office where email = ' petrov85al@mail.ru'), 1, -3, 'самый ужасный фильм в истории кинематографа') , 
--((select id from private_office where email ='golovkovik90@mail.ru'), 2, 2, 'да, нравится фильмы этого режиссёра' ), 
--((select id from private_office where email = 'bolodima88@mail.ru'), 3,  3, 'супер фильм') , 
--((select id from private_office where email = 'fedotovslava94@mail.ru'), 4, 1, 'фильм на один раз'),
--((select id from private_office where email = 'svetlyakovaolya92@mail.ru'), 5,2 , 'фильм норм') ;
--update comments set user_name = (select id from private_office where email = 'petrov85al@mail.ru') where id = 1;
select private_office.id as user_id, private_office.name , private_office.surname , private_office.birth, private_office.email, 
private_office.user_subscription, private_office.payment , private_office.favorite_films, 
subscription.title as title_of_subscription, subscription.description as description_of_subscription,
subscription.price , 
film_list.title, film_list.description, film_list.country, film_list.director, film_list.actors, 
film_list.year, film_list.film_genre, film_list.film_grade, 
grade.title as grade_title, 
news.title, news.news, news.date_of_news, news.film_tag, 
comments.user_name, comments.film, comments.grade, comments.comment,
country.place_name, 
directors.full_name, directors.birthday, directors.reward, 
genre.title_of_genre, genre.description_of_genre 
from comments
